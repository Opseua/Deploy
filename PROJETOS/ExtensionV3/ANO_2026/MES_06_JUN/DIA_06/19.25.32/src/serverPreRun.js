...
async function resourceImport({ endpoint, currentProject = `ExtensionV3`, imports = { 'resources': [], }, attempsMax = 11, attempsDelay = 30, }) {
    ...
    let isGitHub = typeof endpointOriginal === 'string' && endpointOriginal.includes('raw.githubusercontent.com'); endpoint = endpointIsTrue ? localEndpoint : (endpointIsNull ? localEndpoint : endpoint ? endpoint : false);
    ...
    routesCache = routesCache || {}; let now = Math.trunc(Date.now() / 1000); if ((endpointIsTrue || isGitHub) && (!routesCache.body || (now - routesCache.tim) > 60)) {
        try {
            let routesUrl = `${isGitHub ? endpoint : `${localEndpoint}/Deploy`}/routes.json`; let routesRes = await fetch(routesUrl); let routesBody;
