...
async function background() {
    ...
    try {
        ...
        let resources = {}; globalThis[`_resources`] = resources;

        globalThis.executeAST = executeAST; globalThis.acorn = acorn; globalThis.importTypeCss = {}; globalThis.importTypeMedia = {}; globalThis.importTypeJson = {};
        globalThis.window = globalThis.window || globalThis; globalThis.DOMParser = globalThis.DOMParser || class { parseFromString() { return { 'body': { 'innerHTML': '', }, }; } };
        ...
        }; globalThis.localStorage = globalThis.localStorage || { 'getItem': () => null, 'setItem': () => null, 'removeItem': () => null, 'clear': () => null, 'key': () => null, 'length': 0, };
        globalThis.alert = globalThis.alert || ((msg) => console.log(`[Alert Mock]: ${msg}`));
        ...
        globalThis.readFile = async function (p) { try { let r = await fetch(chrome.runtime.getURL(p)); if (!r.ok) { throw new Error('AO LER 1'); } r = await r.text(); return r; } catch { throw new Error('AO LER 2'); } };
        ...
        globalThis.resourcePrepare = async function ({ content, fileType, keepInString, finalVarName, }) {
            if (!content?.includes(`{`)) { content = encryptDecrypt(password, content, false); } if (debug) { console.log(content); }
            if (fileType === 'js') {
                await executeAST(acorn.parse(content, { 'ecmaVersion': 2020, 'sourceType': 'module', }));
            } else if (fileType === 'json') {
                resources[finalVarName] = keepInString ? content : JSON.parse(content);
            }
        };
        ...
        async function getCheckSums(inf = {}) {
            ...
            let res = { 'paths': [], }; for (let [idx, path,] of caminhos.entries()) {
                let item = { path, 'sum': false, }; try { let txt = (path === x) ? mTexto : await _fetch(chrome.runtime.getURL(path)).then(r => r.text()); item.sum = getCrc32(txt); } catch (e) { }
                if (setStartup.has(path)) { item.runAtStartup = true; } res.paths.push(item);
            } master = JSON.parse(await readFile(master)); ret = res.paths; res = res.paths.map(p => p.sum).join(';'); res = [res, (await reduceString(res)),];
        ...
        content = await readFile('src/serverPreRun.js');
