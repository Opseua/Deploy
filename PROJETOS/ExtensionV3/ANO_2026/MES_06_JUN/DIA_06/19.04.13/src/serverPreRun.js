...
let debug = false, infOk; debug = true;
...
async function resourceImport({ endpoint, currentProject = `ExtensionV3`, imports = { 'resources': [], }, attempsMax = 11, attempsDelay = 30, }) {
    let localEndpoint = 'http://127.0.0.1:4321';
    endpoint = endpoint === undefined ? infOk?.master?.endpoint : endpoint;
    let endpointOriginal = endpoint, endpointIsNull = endpoint === null, endpointIsTrue = endpoint === true;
    let isGitHub = typeof endpointOriginal === 'string' && endpointOriginal.includes('raw.githubusercontent.com');
    endpoint = endpointIsTrue ? localEndpoint : (endpointIsNull ? localEndpoint : endpoint ? endpoint : false);
    let delay = (s) => new Promise(r => setTimeout(r, s * 1000)); let resources = {}; globalThis[`_resources`] = resources;

    // [RESOLVER VERSÃO via routes.json] — apenas quando usa Deploy (true ou remoto)
    let version = null;
    if (endpointIsTrue || isGitHub) {
        try {
            let routesUrl = `${isGitHub ? endpoint : `${localEndpoint}/Deploy`}/routes.json`;
            let routesRes = await fetch(routesUrl); let routesBody;
            try { routesBody = await routesRes.json(); } catch { throw new Error('routes.json INVÁLIDO'); }
            if (!routesRes.ok) { throw new Error('routes.json NÃO ENCONTRADO'); }
            let masterKey = infOk?.master?.master;
            routesBody = isGitHub ? routesBody : (routesBody.data ? JSON.parse(routesBody.data) : routesBody);
            version = routesBody?.master?.[masterKey]?.PROJETOS?.[currentProject]?.version;
    ...
    async function tryImportResource(resource) {
        let { 'filePath': rawFilePath, varName = null, keepInString, } = resource;
        let filePath;
        if (!endpoint) { filePath = rawFilePath; }                                                                    // false → disco local SEM Deploy
        else if (endpointIsNull) { filePath = `${currentProject}/${rawFilePath}`; }                                   // null  → servidor local SEM Deploy
        ...
        let response, content, parts = filePath.split('.'), fileType = parts.pop().toLowerCase(), fileName = filePath.split('/').pop().split('.')[0];
        try {
            if (!endpoint) {
                // [LOCAL]
                ...
                if (!response.ok) { throw new Error(`${resBody?.msg || 'ERRO DESCONHECIDO'}`); }
                content = isGitHub ? (typeof resBody === 'string' ? resBody : JSON.stringify(resBody)) : resBody.data;
            }
    ...
    let pendentes = [...imports.resources,];
    for (let attempt = 1; attempt <= attempsMax; attempt++) {
        ...
        if (attempt < attempsMax) {
            if (debug) { console.log(`⏳ [RETRY ${attempt}/${attempsMax}] ${pendentes.length} resource(s) falhou. Aguardando ${attempsDelay}s...`); } await delay(attempsDelay);
        } else {
            console.log(`🚫 [RETRY ESGOTADO] ${pendentes.length} resource(s) não importado(s).`);
        }
    }
