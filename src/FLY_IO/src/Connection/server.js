...
await import('./shared/clientConnect.js'); let debug, portBridge = 8887, ip = '127.0.0.1', port = '8888', enc = new TextEncoder(), dec = new TextDecoder(), nc, tag = `SERVER [node]`, psw = 'SENHA_AQUI';
await globalThis.clientConnect({ wsconnect, 'servers': `ws://${ip}:${port}`, 'label': 'SER', 'onConnect': (c) => { nc = c; }, 'identification': 'BRIDGE', }); debug = 2;
...
function registerLegacy({ identification, socket, }) {
    ...
    socket.on('message', (raw) => {
        try {
            ...
            if (idMessage) {
                ...
                let { replySubject, pending, } = pendingMessages.get(idMessage); let resData = typeof res === 'object' ? res : { 'message': res, };
                let reordered = reorderRes({ 'data': { identification, ...resData, }, 'isReply': true, }); if (pending.has(replySubject)) { pending.get(replySubject)(reordered); pending.delete(replySubject); }
    ...
    (async () => {
        for await (let msg of sub) {
            let data; try { data = JSON.parse(dec.decode(msg.data)); } catch { continue; } let needsReply = data.maxAwait > 0; if (!needsReply && msg.reply) { msg.respond(enc.encode('ok')); } let idMessage = null;
            ...
            socketSend(socket, { 'ret': true, 'msg': `${tag}: OK`, 'res': reorderRes({ data, idMessage, }), }); if (!needsReply || !msg.reply) { continue; }
            await new Promise((resolve) => {
                let timer = setTimeout(() => { pendingMessages.delete(idMessage); pending.delete(msg.reply); resolve(); }, (data.maxAwait * 1000) + 500);
                pending.set(msg.reply, (res) => { clearTimeout(timer); pendingMessages.delete(idMessage); msg.respond(enc.encode(JSON.stringify(res))); resolve(); });
            });
        }
    })();
}
...
async function dispatch({ params, }) {
    ...
    try {
        let resp = await nc.request(`msg.${destination}`, enc.encode(JSON.stringify({ ...params, 'identification': params.identification, })), { timeout, }); if (maxAwait <= 0) { return { 'ret': true, 'msg': `${tag}: OK`, }; }
        let body = JSON.parse(dec.decode(resp.data)); let res = reorderRes({ 'data': { 'identification': params.identification, ...body, }, 'isReply': true, });
