...
async function waitAndNotify({ executables = [], ports = [], scripts = [] }) {
    console.log('[notify] aguardando serviços...'); while (true) {
        ...
        let scriptChecks = scripts.map(s => _loadedScripts.has(s)); console.log('[notify]', JSON.stringify({
            ...
            fetch(`https://ntfy.sh/${process.env.NTFY_CHANNEL}/publish?title=Deploy&message=Conclu%C3%ADdo%20(${engName})`).catch(() => { }); console.log('[notify] ✅ todos os serviços prontos'); break;
        }
    }
}

// ########################### EXECUTÁVEIS
let syncthingZOutros = `${fileWindows}/PORTABLE-Syncthing/z_OUTROS`, zOutros = './src/gitHubActions/z_OUTROS'
startBin({
